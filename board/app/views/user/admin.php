			...
			<tr>
				...
				<td>
					<?php if (($k->type === 'admin') && !($k->id === $_SESSION['id'])): ?>
						<a class="btn btn-mini" href="<?php encode(url('user/admin', array('id' => $k->id, 'type' => 'user'))) ?>" >Demote</a>
					<?php elseif ($k->type === 'banned'): ?>
						<a class="btn btn-mini" href="<?php encode(url('user/admin', array('id' => $k->id, 'type' => 'user'))) ?>" >Unban</a>
					<?php elseif ($k->type === 'user'): ?>
						<a class="btn btn-mini" href="<?php encode(url('user/admin', array('id' => $k->id, 'type' => 'admin'))) ?>" >Promote</a>
						<a class="btn btn-mini" href="<?php encode(url('user/admin', array('id' => $k->id, 'type' => 'banned'))) ?>" >Ban</a>
					<?php endif ?>
				</td>
			</tr>
		<?php endforeach ?>
